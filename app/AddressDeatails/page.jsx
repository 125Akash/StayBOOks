...
const Address = () => {
  ...
  const sendDataToDatabase = async(data) => {
    try {
     const res = await fetch("https://akashpedia-ebaf8-default-rtdb.firebaseio.com/address.json",{
      method: "POST",
   ...
   const fetchDataFromDatabase = () => {
    try {
      const handleData = (snapshot) => {
        ...
        const attributesId = Object.keys(data)[0];
        const addressData = data[attributesId];
      ...
      <div className="flex flex-col justify-center items-center ml-[11%]">
        {Object.keys(initialFields).map((field) => (
         ...
         <div className="flex justify-center items-center m-11">
          <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-full" onClick={() => sendDataToDatabase(fields)}>
            <Link href="/Description">Next</Link>
          </button>
        </div>
      </div>
    </>
